...
def getConfigValue(group, name):
    ...
    if isinstance(default, bool):
        return QtCore.QSettings().value("/Versio/Settings/%s/%s" % (group, name), default, bool)
    else:
        return QtCore.QSettings().value("/Versio/Settings/%s/%s" % (group, name), default, str)

def setConfigValue(group, name, value):
    return QtCore.QSettings().setValue("/Versio/Settings/%s/%s" % (group, name), value)
