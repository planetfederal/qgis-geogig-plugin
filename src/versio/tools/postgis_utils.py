...
class GeoDB:
    ...
    def table_add_geogigid_column(self, schema, table):
        table_name = self._table_name(schema, table)
        try:
            self.table_delete_column(table, "geogigid", schema)
        except DbError, e:
            pass
        sql = "ALTER TABLE %s ADD geogigid text DEFAULT uuid_in(md5(random()::text || now()::text)::cstring)" % (table_name)
        self._exec_sql_and_commit(sql)
