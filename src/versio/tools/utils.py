...
def reposFolder(username):
    return  os.path.join(parentReposFolder(), username)

def repoFolder(username, ownername, reponame):
    return os.path.join(parentReposFolder(), username, ownername, reponame)
...
def userLocalRepos(username):
    'returns a dict with titles as keys and folders as values'
    repos = {}
    parentFolder = reposFolder(username)
    ...
    for subfolder in subfolders:
        subsubfolders = [os.path.join(subfolder, subsubfolder) for subsubfolder
                  in os.listdir(subfolder)
                  if os.path.isdir(os.path.join(subfolder, subsubfolder))]
        for subsubfolder in subsubfolders:
            titlefile = os.path.join(subsubfolder, "title")
            geogigfolder = os.path.join(subsubfolder, ".geogig")
            if os.path.exists(geogigfolder):
                if os.path.exists(titlefile):
                    with open(titlefile) as f:
                        title = f.readline()
                else:
                    title = os.path.basename(subsubfolder)
                repos[title] = subsubfolder
    return repos

def allLocalRepos():
    parentFolder = parentReposFolder()
    usernames = [subfolder for subfolder
                  in os.listdir(parentFolder)
                  if os.path.isdir(os.path.join(parentFolder, subfolder))]
    repos = {username:userLocalRepos(username) for username in usernames}
    return repos
