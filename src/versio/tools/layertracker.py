...
from versio.gui.dialogs.commitdialog import CommitDialog
from versio.tools.versioinstance import config
from versio.gui.pyqtconnectordecorator import createRepository
import logging
...
from versio.tools.layertracking import setInSync, setRef
from versio.tools.layers import getDatabaseCredentials
from geogigpy.py4jconnector import Py4JConnectionException
from versio.gui.dialogs.gatewaynotavailabledialog import GatewayNotAvailableWhileEditingDialog
import uuid
from versio.tools.utils import reposFolder
...
class LayerTracker(object):
    ...
    def doUpdateLayer(self, dest, repo):
        if self.layer.dataProvider().fieldNameIndex("geogigid") == -1:
            config.iface.messageBar().pushMessage("Cannot update Versio repository. Layer has no 'geogigid' field",
