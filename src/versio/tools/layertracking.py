...
from versio.tools.utils import userFolder, repoFolder
import json
from json.decoder import JSONDecoder
from json.encoder import JSONEncoder
from versio.tools.layers import  resolveLayerFromSource, \
...
def decoder(jsonobj):
    if 'source' in jsonobj:
        return TrackedLayer(jsonobj['source'], jsonobj['user'], jsonobj['owner'],
...
class TrackedLayer(object):
    def __init__(self, source, user, owner, reponame, layername, ref, insync):
    ...
    def repoFolder(self):
        return repoFolder(self.user, self.owner, self.reponame)
...
def addTrackedLayer(source, reponame, owner, user, layername, ref):
    global tracked
    source = _formatSource(source)
    layer = TrackedLayer(source, user, owner, reponame, layername, ref, True)
