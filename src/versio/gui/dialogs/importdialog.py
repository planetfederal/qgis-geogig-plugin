...
from versio.tools.layers import *
import os
from geogigpy.geogigexception import GeoGigException, UnconfiguredUserException
from geogigpy import geogig
from versio.gui.dialogs.userconfigdialog import UserConfigDialog
from versio.tools.exporter import exportVectorLayer
from versio.tools.layertracking import addTrackedLayer, isTracked
from versio.tools.postgis_utils import DbError
from versio.gui.dialogs.addgeogigiddialog import AddGeogigIdDialog
from versio.tools.versioinstance import instance
from versio.tools.utils import userLocalRepos, allLocalRepos, userFromRepoPath, \
...
class ImportDialog(QtGui.QDialog):
        ...
        if self.repo is None:
            if isinstance(instance(), LocalVersio):
                ...
                for user, userrepos in repos.iteritems():
                    self.repos.update({"%s" % title:path
                                       for title, path in userrepos.iteritems()})
            else:
                self.repos = userLocalRepos(instance().user)
    ...
    def importClicked(self):
        ...
        user = userFromRepoPath(self.repo.url)
        owner = ownerFromRepoPath(self.repo.url)
        addTrackedLayer(self.layer or source, reponame, owner, user, self.layer.name(), ref)
        self.ok = True
